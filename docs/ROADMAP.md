...
2. Ship V1.5 shared data at `expense.mac-tbo.com` behind Cloudflare Access, using D1/R2 as the durable source while keeping the Vercel V1 app live as fallback, with AgentMail webhook intake for automatic received-email sync, renderer-backed HTML email receipt PDFs, and PDF-only supporting evidence in Export Packages.
